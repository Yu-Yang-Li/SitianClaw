...
description: Forecast the next 3 days of observability for a target across the SNC telescope set by using the same multi-site observability utilities as the workspace. Use when Codex needs the workspace-style forward observability comparison, not just a single-night altitude glance.
---
...
1. Resolve the target coordinates from the workspace caches or direct input.
