...
from sitianclaw_runtime.candidates import build_recent_nearby_pool  # noqa: E402
...
DEFAULT_REDSHIFT_THRESHOLD = 0.025
...
def _source_counts(dataframe: pd.DataFrame) -> dict[str, int]:
    if dataframe.empty:
        return {}
    counts = dataframe["data_source"].fillna("unknown").astype(str).value_counts(dropna=False).to_dict()
...
def _plot_source_mix(source_counts: dict[str, int], output_path: Path) -> Path | None:
    ...
    ax.set_title("SitianClaw 3-Day Candidate Crawl: Source Mix")
...
def _plot_sky_distribution(dataframe: pd.DataFrame, output_path: Path) -> Path | None:
    if dataframe.empty:
        return None
    ra_series = pd.to_numeric(dataframe.get("ra_deg"), errors="coerce")
    dec_series = pd.to_numeric(dataframe.get("dec_deg"), errors="coerce")
    ...
    for idx, (label, group) in enumerate(grouped):
        ax.scatter(group["plot_ra"], group["plot_dec"], s=24, alpha=0.8, label=label, color=palette[idx % len(palette)])
    ax.legend(loc="best", fontsize=8)
    ax.set_title("SitianClaw 3-Day Candidate Crawl: Sky Distribution")
...
def _plot_nearby_redshift_histogram(dataframe: pd.DataFrame, output_path: Path) -> Path | None:
    ...
    ax.set_title("SitianClaw Nearby Candidate Pool: Redshift Distribution")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fetch the SitianClaw 3-day candidate pool from TNS, ZTF, and LSST, then build a nearby-universe pool."
    )
    ...
    parser.add_argument(
        "--nearby-redshift-max",
        type=float,
        default=DEFAULT_REDSHIFT_THRESHOLD,
        help="Nearby-universe redshift ceiling for the candidate pool.",
    )
    ...
    result, recent_df, nearby_pool = build_recent_nearby_pool(
    ...
    nearby_redshift_plot = _plot_nearby_redshift_histogram(
        nearby_pool, output_dir / "candidate_crawl_3day_nearby_redshift.png"
    )

    payload: dict[str, Any] = {
        "skill": "snc-candidate-crawl-3day",
        "workspace_alignment": "GitHub-only portable candidate crawl with the same 3-day intake intent and nearby-first redshift threshold.",
        "parameters": {
            "include_ztf": not args.skip_ztf,
            "include_lsst": not args.skip_lsst,
            "ztf_days_back": args.ztf_days_back,
        ...
        "preview": dataframe_records(
            combined_data,
            columns=["tns_name", "type", "discoverydate", "discoverymag", "data_source", "ra_deg", "dec_deg"],
            limit=30,
        ),
        "artifacts": {
            "csv": str(csv_path),
